...
description: Procedural workflow to translate Anki note fields into English. Use when an AI agent needs to enrich Anki decks with English translations. Supports parameters for deck selection, field mapping, and batch control.
---
...
- `--batch-size`: (Optional) The number of unique sentences to translate in a single evaluation request. Defaults to `5`.
...
2. **Normalize**: Remove Anki cloze markers using regex or similar logic: `re.sub(r"\{\{c\d+::(.*?)(::.*?)?\}\}", r"\1", text)`.
3. **Deduplicate**: Identify unique normalized strings to minimize translation overhead.

### 3. Translate
Translate the unique strings to English.
...
- **Batching**: Respect the `--batch-size` when translating strings.
